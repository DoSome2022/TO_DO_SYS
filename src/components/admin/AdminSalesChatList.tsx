...
export default function AdminSalesChatList({
  ...
  const filteredChannels = useMemo(() => {
    ...
    return channels.filter((ch) => {
      // ✨ 修正：title 和 status 直接在 ch 上，沒有 ch.project
      const matchesSearch = !searchTerm
        ? true
    ...
    <div className="w-[380px] flex-shrink-0 border-r border-gray-200 bg-white flex flex-col">
      ...
      <ScrollArea className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-100">
            {filteredChannels.map((channel) => {
              // ✨ 修正：用 channel.id 比對（或 channel.projectId 若有此欄位）
              const isActive = channel.id === selectedProjectId;
              // ✨ 修正：messages[0] → lastMessage
              ...
              return (
                <button
                  key={channel.id}
                  onClick={() =>
                    // ✨ 修正：channel.project.title → channel.title
                    onSelectProject(channel.id, channel.title)
                  }
                  className={`w-full text-left px-4 py-3 transition-colors hover:bg-gray-50
                    ${isActive ? "bg-blue-50 border-l-2 border-l-blue-500" : ""}
                  `}
