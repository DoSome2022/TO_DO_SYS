...
// // src/components/staff/stafftode/stafftodomanager.tsx
// "use client";
...
// import { RotateCcw } from "lucide-react"; // 🆕 如果還沒有的話
...
//                                         billingType: log.equipment?.billingType,
//                                         price: log.equipment?.price,
//                                       });
//                                       setReturnDialogOpen(true);
//                                     }}
//                                     className="p-1 text-green-600 hover:bg-green-50 rounded text-[10px]"
//                                     title="歸還設備"
//                                   >
  ...
  const handleEdit = (todo: any) => {
    setEditingTodo({
      ...
      targetDate: todo.targetDate
        ? new Date(todo.targetDate).toISOString().split("T")[0]
        : "",
    });
    setIsDialogOpen(true);
  };
    ...
    <div className="max-w-3xl mx-auto my-8 bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="p-2">
        {todos?.length === 0 ? (
          ...
          <div className="space-y-1">
            {todos?.map((todo) => {
              const hasEquipmentLogs =
                (todo as any).equipmentLogs?.length > 0;

              return (
                <div key={todo.id}>
                  {/* 主要任務卡片 */}
                  <div
                    className={`group flex items-center p-3 rounded-lg hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100 ${
                            ...
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  ...
                                  {log.equipment?.billingType !== "NONE" &&
                                    log.equipment?.price > 0 && (
                                      <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded">
                                        <DollarSign className="w-2.5 h-2.5 inline" />
                                        ${log.equipment.price}/
                                        {log.equipment.billingType ===
                                        "HOURLY"
                                          ? "hr"
                                          : "day"}
                                      </span>
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-[10px] text-gray-400">
                                    {format(
                                      new Date(log.borrowedAt),
                                      "MM/dd"
                                    )}
                                    {isReturned &&
                                      `→${format(
                                        new Date(log.returnedAt),
                                        "MM/dd"
                                      )}`}
                                  </span>
