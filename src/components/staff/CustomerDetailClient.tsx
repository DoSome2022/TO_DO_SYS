...
// 🆕 引入報價單詳情元件（與報價單列表頁共用的 Dialog）
import { QuotationDetailClient } from "@/components/sales/quotations/QuotationDetailClient";
...
export default function CustomerDetailClient({
  ...
  // 獲取客戶的報價單列表
  const { data: quotations, refetch: refetchQuotations } = trpc.staff.getCustomerQuotations.useQuery(
    { customerId, staffId },
    { enabled: activeTab === "quotations" }
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {activeTab === "info" &&
          (isEditing ? (
            <CustomerInfoEdit customer={customer} onCancel={handleCancelEdit} onSuccess={handleEditSuccess} />
          ) : (
            <CustomerInfoView customer={customer} onEdit={handleStartEdit} />
          ))}
      ...
      {/* 🆕 報價單詳情彈窗 — 與報價單列表頁共用同一元件 */}
      <Dialog
        open={!!selectedQuotationId}
        onOpenChange={(open) => {
          if (!open) {
            setSelectedQuotationId(null);
            refetchQuotations(); // 關閉彈窗時刷新列表
          }
        }}
      >
